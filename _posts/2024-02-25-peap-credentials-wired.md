...
## If the victim downloads our file that looking legitimite
...
The third parameter is for the icon. I assume that you want to convert file icon to PDF. In the classic way, after running x86matthew's script, 
you would __right-click -> Properties -> Change Icon -> Put the msedge.exe location__ ('C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe') -> OK. Is that right? That's okay. But when you select the PDF icon and click OK, the lnk file structure will break and it will not run. So you can not use this. 
...
When you go to byte 56, you will see that first lnk file has 0x00, second lnk file has 0x0D (13) index number. The thing you just gonna do is to change this byte to 0x0D. So you can change icon to pdf.
