...
If you look at line 126, you will see that first the lnk file finds itself by its length, then it take itself as bytes and finally dexoring. As last, without touch a disk and go to the internet it's to be running powershell code in memory.

There are some tricks in scripts while creating lnk file. They are:

-Not going to the internet for powershell code.
-Not touching to the disk
-it' storing ps code in itself with xored. So it's encrytped.
-As you can see, it's running ps code in memory.
...
Of course, There are also some another options. You can embed zip file at the last lnk file. And then again unraring and run code etc.
